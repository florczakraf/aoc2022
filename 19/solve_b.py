...
def maximize_geode(bp, timeout):
    ...
    while not q.empty():
        ...
        state.t += 1
        if state.t > timeout:
            max_score = max(max_score, state.geode)
            continue
        ...
        if state.geode > max_after[state.t]:
            max_after[state.t] = state.geode
        if state.geode + 2 < max_after[state.t]:  # magic, do not touch (works for my input)
            continue
...
for line in sys.stdin.read().splitlines()[:3]:
    ...
    total *= maximize_geode(blueprint, 32)
