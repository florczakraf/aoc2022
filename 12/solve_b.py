...
def neighbors(v):
    ...
    candidates = [c for c in candidates if 0 <= c[0] < max_y and 0 <= c[1] < max_x and map[c[0]][c[1]] + 1 >= map[y][x]]
...
print(sorted(distances.get((y, x), math.inf) for y in range(len(map)) for x in range(len(map[0])) if map[y][x] == 0)[0])
